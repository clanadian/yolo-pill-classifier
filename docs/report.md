...
> 이 저하는 특정 보드(.107)에서만 재현됐고, 카드 스왑 테스트로 보드 하드웨어는 무죄로
> 확인됐다 — 원인은 그날 그 카드의 상태로 좁혀짐. 아래 "Board Variance" 참고.
...
### 카드 스왑 테스트로 원인 확정 (2026-08-13, 후속)

`.107`에 물리적으로 접근 가능해져서, 결정적인 격리 테스트를 했다: **`.107` 보드 하드웨어에 `.108`의 (이미 안정성이 확인된) SD카드를 꽂아서 부팅**했다.

* 새 부팅 확인(uptime 1분, hostname `nano18`), `.107` 하드웨어 위에서 `.108`의 카드로 실행
* 110초 연속 측정 — **6.2~6.5 FPS로 저하 없이 안정**(`.108` 원래 조합의 7.0~7.4 FPS보다 소폭 낮지만, 시간에 따른 하락 패턴은 전혀 없음)
* RAM도 동일하게 3.26~3.32GB(약 82~84%)까지 상승 후 유지, GPU 온도 25~29.5°C — 지금까지와 같은 패턴
* SD카드 자체 벤치마크(dd, drop_caches 후): 쓰기 23.5MB/s / 읽기 84.4MB/s — 이 카드가 원래 보드(`.108`)에서 측정했을 때(쓰기 24.6MB/s / 읽기 82.0MB/s)와 거의 동일 → **카드 성능은 어느 보드에 꽂든 일관됨**, 보드의 SD 컨트롤러 차이도 아님
* draw_ms만 다소 높게 나옴(평균 70.26ms vs `.108` 원래 조합 49.55ms, inference_ms는 63ms대로 거의 동일) — 보드별 미세한 렌더링/드라이버 차이로 보이나 fps 저하(시간에 따른 하락)와는 무관
* 패키지 버전도 리셋 전 `.107`(대화 초반 기록)과 대조: Python 3.8.10 / torch 1.13.0a0+git7c98e70(JetPack 4.6 전용 빌드) / opencv 5.0.0 / ultralytics 8.3.0 — **한 글자도 다르지 않음.** 소프트웨어 버전 드리프트도 원인에서 배제.

**결론**: `.107` 보드 하드웨어는 무죄로 확정됐다 — 검증된 카드를 꽂으면 그 보드에서도 저하 없이 안정적으로 돈다. 즉 6번 항목에서 기록한 원래의 FPS 저하는 **`.107` 보드 자체의 문제가 아니라, 그날(2026-07-28) `.107`에 꽂혀 있던 원래 SD카드(현재는 리셋되어 재현 불가능)의 그 시점 상태에 국한된 현상**이었을 가능성이 가장 높다. 정확한 원인(카드 자체의 결함/노후화, 파일시스템 단편화, 혹은 그 카드에만 있던 백그라운드 프로세스 등)은 카드가 이미 리셋되어 추가 확인이 불가능하므로 미해결로 남긴다.
