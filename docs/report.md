...
- **회귀 발견 및 되돌림 (2026-07-29)**: `pink_caplet` 검은 배경 72장만 육안으로 다시 보니 채도가 거의 사라져 회색에 가깝게 보임. 픽셀 실측 결과 보정 전 R-G 채널 차이 약 30(정상적인 핑크)이던 것이 보정 후 약 5(거의 무채색)로 오히려 채도가 줄어듦 — 다른 5개 클래스(green/yellow/mint/white/capsule)는 정상. `COLOR/` 원본·보정본 참조 사진이 이미 삭제돼 재학습 불가하여, 문제의 72장은 보정 전(원본, git 커밋 `77564d3`) 상태로 되돌림. 따라서 **현재 `pink_caplet`은 색감 보정 미적용 상태**이며, 정확한 회귀 원인(LUT 방향 반전 추정)은 미확인으로 남음.
