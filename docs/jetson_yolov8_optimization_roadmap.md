...
- 시작 직후 5~7 FPS, 약 1분 후 3.3~4.3 FPS (**단, 이 저하는 특정 보드(.107)에서만
  재현됨 — 아래 "보드 편차" 항목 참고. 다른 보드(.108)에서는 110초 동안 7.0~7.4
  FPS로 전혀 안 떨어짐**)
- CPU 한 코어 지속 점유, GPU 사용률 간헐적 상승
- RAM 약 3.1~3.2 GB로 상승 후 정체 (두 보드 모두 동일 패턴 — RAM은 fps 저하의
  원인이 아닌 것으로 확인됨, `docs/report.md`의 "Board Variance")
- 단계별 latency 측정 결과(.108, 2026-08-13, 492프레임): `draw_ms` 평균
  49.55ms로 `inference_ms`(63.16ms)의 78%, `total_ms`(136.86ms)의 36%를 차지 —
  예상보다 훨씬 큼. TensorRT로 inference만 줄이면 `draw_ms`가 새 병목이 될 가능성이
  높으므로 필수 3 비교 시 반드시 같이 봐야 한다.
- **보드 편차 (해결됨)**: 원래 FPS 저하를 기록한 보드(.107)의 SD카드가 이후
  리셋됐고, 같은 조건을 다른 보드(.108)에서 재현했더니 저하가 재현되지 않았다.
  이후 `.108`의 (안정성이 확인된) SD카드를 `.107` 보드 하드웨어에 꽂아 부팅하는
  카드 스왑 테스트로 확정했다 — **`.107` 보드에서도 검증된 카드로는 110초 동안
  6.2~6.5 FPS로 저하 없이 안정적이었다.** 즉 `.107` 보드 하드웨어는 무죄이고,
  원래 저하는 그날 `.107`에 꽂혀 있던 (현재는 리셋되어 재확인 불가능한) 카드의
  그 시점 상태에 국한된 문제였을 가능성이 가장 높다(`docs/report.md`의
  "Board Variance" 참고). **다만 정확한 근본 원인(그 카드의 결함/노후화 등)은
  카드가 리셋되어 더 확인할 수 없으므로 미해결로 남는다 — Baseline/TensorRT
  비교는 지금 접근 가능한 보드+카드 조합 아무거나로 진행해도 되지만, 수치가
  보드/카드 조합에 따라 달라질 수 있다는 전제는 계속 염두에 둔다.**
...
- 3회 반복
- 카드 스왑 테스트로 `.107` 보드 하드웨어는 무죄로 확인됐으므로(위 "보드 편차" 참고),
  지금 접근 가능한 보드+카드 아무 조합으로 baseline을 진행해도 된다. 다만 결과표에는
  어떤 보드/카드 조합에서 측정했는지 반드시 같이 적어 재현 조건을 남긴다.
