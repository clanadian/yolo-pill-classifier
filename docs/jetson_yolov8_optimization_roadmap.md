...
- **보드 편차**: 원래 FPS 저하를 기록한 보드(.107)의 SD카드가 이후 리셋됐고,
  같은 조건을 다른 보드(.108)에서 재현했더니 저하가 재현되지 않음. 코드 diff로
  두 시점의 `stream/server.py`/`detector.py`가 계측 코드 추가 외엔 동일함을
  확인했고, RAM/GPU온도/전력모드도 두 보드가 동일한데 fps 패턴만 다름 — 원인은
  SD카드 I/O 개별 편차, 전원 공급 품질, 또는 그 보드에만 있던 배경 프로세스 중
  하나로 추정되나 `.107`에 물리적으로 접근 못 해 미확정 (`docs/report.md`의
  "Board Variance"). **Baseline/TensorRT 비교는 반드시 같은 보드에서, 가능하면 두 보드
  모두에서 돌려서 보드 개별 편차와 최적화 효과를 헷갈리지 않도록 한다.**
...
- 3회 반복
- **보드 편차가 확인된 이상, 최소 1회는 `.107`(리셋 후, 물리 접근 가능해지는 대로)에서도
  돌려서 저하가 재현되는지 봐야 한다. `.108`에서만 3회 돌리고 "baseline"으로 확정하면
  안 됨.**
